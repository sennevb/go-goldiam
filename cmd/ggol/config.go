...
func loadConfig(file string, cfg *ggolConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "ggol.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, ggolConfig) {
	// Load defaults.
	cfg := ggolConfig{
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		utils.Fatalf("Failed to register the Ggol release oracle service: %v", err)
	}
	return stack
}
