...
const (
	clientIdentifier = "ggol" // Client identifier to advertise over the network
)

var (
	// Git SHA1 commit hash of the release (set via linker flags)
	gitCommit = ""
	// Ethereum address of the Ggol release oracle.
	relOracle = common.HexToAddress("0x0")
	// The app that holds all commands and flags.
	app = utils.NewApp(gitCommit, "the go-goldiam command line interface")
...
// ggol is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func ggol(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) {
		...
		if err := stack.Service(&ethereum); err != nil {
			utils.Fatalf("goldiam service not running: %v", err)
		}
